...
public class DHeap
{
    public static void main(String[] args)
    {
        ...
        do
        {
            System.out.println("\nPress 1) for insert, 2) for deleteMin, 3) for new d value, 4) for remove of key 5) for changing a value 6) to quit");
            ...
            switch (choice)
            {
                ...
                case 4 : // delete key
                    ...
                    if(!found)
                        System.out.printf("Cannot delete key %d\n", del);    
                    dh.printHeap();
                    break;
                case 5 : // change value
                    System.out.print("Enter key that you want to change: ");
                    int key=scan.nextInt();
                    boolean f = false;
                    for(int e=0; e<dh.getSize();e++) {
                        if (dh.getKey(e)==key) {
                            f = true;
                        }
                    }
                    if(!f) {
                        System.out.printf("Cannot find key %d to change\n", key);
                        break;
                    }
                    System.out.print("Enter new value for key: ");
                    int newkey=scan.nextInt();
                    dh.changeValue(key,newkey);
                    dh.printHeap();
                    break;
                case 6 :
...
class DHeapClass
{
    ...
    public void changeValue(int val, int newval) 
    {
