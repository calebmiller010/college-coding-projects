...
void io_completed(void)
{
    int i;
    for(i=0;i<num_proc;i++) {
        if(proc[i].state == BLOCKED && proc[i].t_event == t) {
            proc[i].state = READY;
        }
    }
}
...
void schedule_cpus(void)
{
    ...
    for(i=0;i<num_cpu;i++) {
        if(cpu[i].proc_ndx == -1) {
            if(proc[0].state == READY) {
                proc[0].state = RUNNING;
                cpu[i].proc_ndx = 0;
            }
            else {
                proc[proc[0].next_ready].state = RUNNING;
                cpu[i].proc_ndx = proc[0].next_ready;
            }
        }
    }
}
...
void do_preempt(void)
{
    ...
    for(i=0;i<num_proc;i++) {
        if(proc[i].state == RUNNING) {
            if(proc[i].prio < lowest) {
                lowest = proc[i].prio;
                lowestindex = i;
                lowestrt = proc[i].tot_cpu;
            }
            if(proc[i].prio == lowest) {
                //change to CURRENT CPU, NOT TOTAL CPU
                if(proc[i].tot_cpu < lowestrt) {
                    lowest = proc[i].prio;
                    lowestindex = i;
                    lowestrt = proc[i].tot_cpu;
                }
    ...
    int highestreadyprio = -1;
    int highestreadyprioindex;
    for(a=0;a<num_proc;a++) {
        if(proc[a].state == READY) {
            if(proc[a].prio > highestreadyprio) {
                highestreadyprio = proc[a].prio;
                highestreadyprioindex = a;
            }
        }
    }

    if(lowest < highestreadyprio) {
        proc[lowestindex].state = READY;
        proc[highestreadyprioindex].state = RUNNING;
        for(a=0;a<num_cpu;a++) {
            if(cpu[a].proc_ndx == lowestindex) {
                cpu[a].proc_ndx = highestreadyprioindex;
            }
        }
    }
}
